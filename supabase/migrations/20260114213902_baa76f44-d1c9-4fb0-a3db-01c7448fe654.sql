-- Cleanup ambiguous overloads of public.admin_create_contest, keep only canonical signature

-- Drop legacy versions
DROP FUNCTION IF EXISTS public.admin_create_contest(text,bigint,integer,timestamptz,jsonb);
DROP FUNCTION IF EXISTS public.admin_create_contest(text,integer,integer,timestamptz,jsonb,text);
DROP FUNCTION IF EXISTS public.admin_create_contest(jsonb,integer,text,integer,text,character varying,jsonb);
DROP FUNCTION IF EXISTS public.admin_create_contest(text,timestamptz,integer,integer,jsonb,text,jsonb);

-- Re-create canonical function (only remaining version)
...
BEGIN
  ...
  -- Calculate guaranteed prize pool (sum of cents)
